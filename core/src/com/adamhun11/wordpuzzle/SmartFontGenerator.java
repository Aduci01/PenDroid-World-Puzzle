...
public class SmartFontGenerator {
    ...
    public SmartFontGenerator() {
        ...
        pageSize = 512; // size of atlas pages for lvlFont pngs
    }


    /** Will load lvlFont from file. If that fails, lvlFont will be generated and saved to file.
     * @param fontFile the actual lvlFont (.otf, .ttf)
     * @param fontName the name of the lvlFont, i.e. "arial-small", "arial-large", "monospace-10"
     *                 This will be used for creating the lvlFont file names
     * @param fontSize size of lvlFont when screen width equals referenceScreenWidth */
    public BitmapFont createFont(FileHandle fontFile, String fontName, int fontSize) {
        ...
        if (displayWidth != Gdx.graphics.getWidth() || displayHeight != Gdx.graphics.getHeight()) {
            ...
            try {
                ...
                Gdx.app.debug(TAG, "Loading generated lvlFont from file cache");
        ...
        if (!loaded || forceGeneration) {
            ...
            // on later startups, which will require lvlFont regeneration
    ...
    /** Convenience method for generating a lvlFont, and then writing the fnt and png files.
     * Writing a generated lvlFont to files allows the possibility of only generating the fonts when they are missing, otherwise
    ...
    private void saveFontToFile(BitmapFont font, int fontSize, String fontName, PixmapPacker packer) {
        ...
        Gdx.app.debug(TAG, String.format("Saving lvlFont [%s]: fontfile: %s, pixmapDir: %s\n", fontName, fontFile, pixmapDir));
     ...
     * Then the fontSize paramater will be unaltered when creating a lvlFont.  If the screen width is 720, the lvlFont size
     ...
     * by the lvlFont generator.  In the mean time it must be set manually. */
